...
3. ~~**TD-0017** — public marketing claims (device fingerprinting, "300+ signals", proxy/VPN detection) materially exceed real capability.~~ **Resolved 2026-08-08 by copy correction** (together with TD-0056, AML/KYC). The capability gaps themselves (TD-0009, TD-0010, TD-0042) remain open.
...
| TD-0017 | Marketing claims exceed real capability | Documentation / Product / Legal | 4 | `src/pages/Landing.tsx` — "300+ signals", persistent cross-browser fingerprinting, emulator/rooted-device detection | Real signal count is 17 (engine) / 20 (feature store); no fingerprinting exists at all | High — potential misleading-advertising exposure | **P0** | Low (copy) or High (build) | Copy: hours. Build: see TD-0009/0010 | None | Either adjust marketing copy now, or accelerate TD-0009/TD-0010 first | **Resolved (2026-08-08, copy)** — `Landing.tsx`: "300+" → "20" in the hero stat, the "Built for scale" strip and the RiskScore module copy; DeviceID descriptor rewritten to device linking / prior-block / multi-account reuse / headless automation. Resolved by copy only — the underlying capability gap (TD-0009 IP Intelligence, TD-0010 Device Fingerprinting) remains open, so re-verify this row before any future claim of fingerprinting. |
...
| TD-0056 | Marketing claims "AML, KYC" capability with zero implementation | Documentation | 5 | `src/pages/Landing.tsx:355` — Fintech & Banking vertical descriptor | Same overclaim pattern as TD-0017, now confirmed for the Compliance Cloud domain | High — credibility/legal exposure | High | **P0** | Low (copy) | Hours | None | Adjust marketing copy or accelerate Compliance Cloud Phase 1 (Vol. 5 Sec. 25) | **Resolved (2026-08-08, copy)** — `Landing.tsx` Fintech & Banking descriptor changed from "AML, KYC, payment fraud, synthetic identity" to "Payment fraud, account takeover, synthetic identity". No regulated-capability claim remains. Compliance Cloud is still unbuilt (TD-0042), so re-verify before re-introducing any AML/KYC wording. |
...
### 2026-08-08 — Frontend design system pass
- **Origin:** full UI/UX systematisation of the existing product (design tokens, shared primitives, screen hierarchy). Marketing copy was audited against the Volume 4 source-code findings while editing `Landing.tsx`.
- **Items added:** none.
- **Items resolved:** **TD-0017** (signal-count and device-fingerprinting overclaims) and **TD-0056** (AML/KYC overclaim) — both resolved by copy correction only, not by building the capability.
- **Items removed:** none.
- **Register status:** 59 total, 2 resolved, 57 open.
- **Caveat:** TD-0009 (IP Intelligence), TD-0010 (Device Fingerprinting) and TD-0042 (Compliance Cloud) are the capability gaps behind both claims and are still open. Any future marketing copy asserting fingerprinting, proxy/VPN detection, AML or KYC must re-open these rows.
